import {
  collection,
  getDocs
} from 'https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js';

import { db } from '../js/firebase-init.js';
import { runIntentPipeline } from '../packages/icf/pipeline/runIntentPipeline.js';
import {
  getRevenueSummary,
  getSeatSummary,
  getTemplate
} from '../packages/domain/circles/circleMarketplace.js';
import { escapeHtml } from '../packages/ui/renderHelpers.js';

export async function renderCirclesPanel(container) {
  container.innerHTML = `
    <h2 class="text-2xl font-bold mb-4">🪑 Cafe Circles</h2>

    <div class="mb-4 flex flex-wrap gap-4">
      <input id="eventFilter" type="text" placeholder="Search by event..." class="border px-3 py-2 rounded w-60">
      <input id="hostFilter" type="text" placeholder="Search by host..." class="border px-3 py-2 rounded w-60">
      <input id="dateFilter" type="date" class="border px-3 py-2 rounded">
      <select id="statusFilter" class="border px-3 py-2 rounded">
        <option value="">All Statuses</option>
        <option value="upcoming">Upcoming</option>
        <option value="completed">Completed</option>
        <option value="canceled">Canceled</option>
      </select>
    </div>

    <div id="circleList" class="space-y-4"></div>
  `;

  const circles = await loadAllCircles();
  renderCircleList(circles);

  document.getElementById('eventFilter').addEventListener('input', function() {
    filterAndRender(circles);
  });
  document.getElementById('hostFilter').addEventListener('input', function() {
    filterAndRender(circles);
  });
  document.getElementById('dateFilter').addEventListener('change', function() {
    filterAndRender(circles);
  });
  document.getElementById('statusFilter').addEventListener('change', function() {
    filterAndRender(circles);
  });
}

async function loadAllCircles() {
  const snap = await getDocs(collection(db, 'cafeCircles'));
  const circles = [];
  snap.forEach(function(circleDoc) {
    circles.push({ id: circleDoc.id, ...circleDoc.data() });
  });
  return circles;
}

async function requireIntentSuccess(result, fallbackMessage) {
  if (!result || !result.ok) {
    throw new Error(result && result.message ? result.message : fallbackMessage);
  }
  return result;
}

function renderCircleList(circles) {
  const container = document.getElementById('circleList');
  container.innerHTML = '';

  circles.forEach(function(circle) {
    const div = document.createElement('div');
    div.className = 'bg-white p-4 rounded shadow border';

    const date = circle.date ? new Date(circle.date.seconds * 1000).toLocaleString() : 'N/A';

    div.innerHTML = `
      <div class="flex justify-between items-start">
        <div>
          <h3 class="text-lg font-bold mb-2">${escapeHtml(circle.title || 'Unnamed Event')}</h3>
          <p><strong>Host:</strong> ${escapeHtml(circle.hostName || circle.hostId || 'Unassigned')}</p>
          <p><strong>Date:</strong> ${date}</p>
          <p><strong>Status:</strong> ${escapeHtml(circle.status || 'upcoming')}</p>
          ${renderCircleRevenueFacts(circle)}
          <p class="text-sm mt-2 text-gray-700">${escapeHtml(circle.description || '')}</p>
          <details class="mt-3 bg-gray-50 rounded p-3">
            <summary class="cursor-pointer font-semibold">Circle detail</summary>
            ${circle.image ? `<img src="${escapeHtml(circle.image)}" alt="${escapeHtml(circle.title || 'Circle')}" class="w-40 rounded mt-3">` : ''}
            <div class="grid md:grid-cols-3 gap-2 mt-3 text-sm">
              <span>Gifts: ${circle.budget?.gifts || 0} som</span>
              <span>Food: ${circle.budget?.food || 0} som</span>
              <span>Host salary: ${circle.budget?.salary || 0} som</span>
              <span>Marketing: ${circle.budget?.marketing || 0} som</span>
              <span>Venue: ${circle.budget?.venue || 0} som</span>
              <span>Partner share: ${circle.cafePartner?.sharePercent || 0}%</span>
            </div>
            <div class="mt-3">
              <strong>RSVP statuses</strong>
              ${(circle.rsvp || []).map(function(person) { return `<p>${escapeHtml(person.name || person.userId || 'Guest')} · ${escapeHtml(person.status || 'confirmed')}</p>`; }).join('') || '<p>No RSVPs yet.</p>'}
            </div>
            <label class="block mt-3 text-sm">Reassign host
              <input class="host-id-input border px-2 py-1 rounded w-full mt-1" value="${escapeHtml(circle.hostId || '')}">
            </label>
            <label class="block mt-3 text-sm">Internal notes
              <textarea class="notes-input border px-2 py-1 rounded w-full mt-1" rows="3">${escapeHtml(circle.internalNotes || '')}</textarea>
            </label>
            <div class="grid md:grid-cols-2 gap-2 mt-3">
              <label class="text-sm">Amount paid
                <input class="payout-input border px-2 py-1 rounded w-full mt-1" type="number" value="${circle.payout?.amountPaid || 0}">
              </label>
              <label class="text-sm flex items-end gap-2">
                <input class="paid-input" type="checkbox" ${circle.payout?.paid ? 'checked' : ''}>
                Paid
              </label>
            </div>
            <button class="save-detail-btn bg-blue-600 text-white px-3 py-1 rounded mt-3">Save detail</button>
          </details>
        </div>
        <div class="flex flex-col gap-2 text-right">
          <button class="bg-blue-600 text-white px-3 py-1 rounded edit-btn">✏️ Edit</button>
          <button class="bg-red-600 text-white px-3 py-1 rounded cancel-btn">🛑 Cancel</button>
        </div>
      </div>
    `;

    div.querySelector('.cancel-btn').addEventListener('click', async function() {
      if (confirm('Cancel this event?')) {
        await requireIntentSuccess(await runIntentPipeline('AdminUpdateCircleIntent', {
          circleId: circle.id,
          circlePatch: { status: 'canceled' }
        }), 'Circle could not be canceled.');
        div.remove();
      }
    });

    div.querySelector('.edit-btn').addEventListener('click', function() {
      div.querySelector('details')?.setAttribute('open', 'open');
    });

    div.querySelector('.save-detail-btn').addEventListener('click', async function() {
      await requireIntentSuccess(await runIntentPipeline('AdminUpdateCircleIntent', {
        circleId: circle.id,
        circlePatch: {
          hostId: div.querySelector('.host-id-input').value.trim(),
          internalNotes: div.querySelector('.notes-input').value.trim(),
          payout: {
            amountPaid: Number(div.querySelector('.payout-input').value || 0),
            paid: div.querySelector('.paid-input').checked
          }
        }
      }), 'Circle detail could not be saved.');
    });

    container.appendChild(div);
  });
}

function renderCircleRevenueFacts(circle) {
  const seats = getSeatSummary(circle);
  const revenue = getRevenueSummary(circle);
  const template = getTemplate(circle);
  return `
    <p><strong>Template:</strong> ${escapeHtml(template.name)}</p>
    <p><strong>Partner cafe:</strong> ${escapeHtml(circle.cafePartner?.name || 'Not set')}</p>
    <p><strong>Ticket:</strong> ${Number(circle.pricePerSeat || 0)} som · <strong>Seats:</strong> ${seats.attending}/${seats.capacity || 'open'}</p>
    <p><strong>Gross:</strong> ${revenue.grossRevenue} som · <strong>Estimated payout:</strong> ${revenue.estimatedPayout} som · <strong>Paid:</strong> ${revenue.paidOut} som</p>
    <p><strong>Budget total:</strong> ${Number(circle.budget?.total || circle.budget || 0)} som</p>
  `;
}

function filterAndRender(allCircles) {
  const event = document.getElementById('eventFilter').value.toLowerCase();
  const host = document.getElementById('hostFilter').value.toLowerCase();
  const date = document.getElementById('dateFilter').value;
  const status = document.getElementById('statusFilter').value;

  const filtered = allCircles.filter(function(circle) {
    const matchEvent = !event || (circle.title || '').toLowerCase().includes(event);
    const matchHost = (circle.hostName || '').toLowerCase().includes(host) || (circle.hostId || '').includes(host);
    const circleDate = circle.date?.seconds ? new Date(circle.date.seconds * 1000).toISOString().slice(0, 10) : '';
    const matchDate = !date || circleDate === date;
    const matchStatus = !status || circle.status === status;
    return matchEvent && matchHost && matchDate && matchStatus;
  });

  renderCircleList(filtered);
}
