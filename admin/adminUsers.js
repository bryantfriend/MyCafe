...
import { closeModal, openModal } from '../packages/ui/modalStack.js';
...
async function saveUserFromModal() {
  ...
  try {
    if (modalMode === 'add') {
      ...
      userData.reviews = [];
      userData.badges = [];
      const addResult = await runIntentPipeline('AdminSaveUserIntent', {
        mode: 'add',
        userData
      });
      if (!addResult.ok) {
        throw new Error(addResult.message || 'User could not be added.');
      }
    } else if (modalMode === 'edit' && currentEditUserId) {
      const editResult = await runIntentPipeline('AdminSaveUserIntent', {
        mode: 'edit',
        userId: currentEditUserId,
        userData
      });
      if (!editResult.ok) {
        throw new Error(editResult.message || 'User could not be saved.');
      }
    }
...
function renderUserRows(users) {
  ...
  if (isMobile) {
    ...
    users.forEach(user => {
      ...
      card.innerHTML = `
        <div class="font-bold text-lg">${escapeHtml(user.nickname || '(no name)')}</div>
        ...
        <div class="text-sm">👤 ${escapeHtml(user.role || 'user')} | ${user.flagged ? '🚫 Banned' : '✅ Active'}</div>
    ...
    users.forEach(user => {
      ...
      tr.innerHTML = `
        <td class="p-3">${escapeHtml(user.nickname || '(no name)')}</td>
        <td class="p-3">${escapeHtml(user.email)}</td>
        <td class="p-3">${escapeHtml(user.role || 'user')}</td>
...
async function handleDeleteUser(userId) {
  ...
  try {
    const deleteResult = await runIntentPipeline('AdminDeleteUserIntent', { userId });
    if (!deleteResult.ok) {
      throw new Error(deleteResult.message || 'User could not be deleted.');
    }
    renderUsersPanel(document.getElementById('adminContent'));
  } catch (err) {
      ...
      <div class="grid gap-2">
        ...
        ${reviews.map(review => `
          <div class="bg-white border rounded p-2">
            <p>${escapeHtml(review.text || '(empty review)')}</p>
