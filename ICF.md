...
# Intent Minimalism Rules

Intent files are declarations, not implementations.

The purpose of an Intent file is to:

1. Define the Intent type
2. Define basic Intent metadata
3. Reference stage functions
4. Export the Intent

Intent files should be easy to scan in under 30 seconds.

If an Intent exceeds roughly 50-100 lines, consider moving logic into stage files.

Intent files should not contain:
...
- Inline processing code

Those belong inside stage files.

The Intent should primarily act as a stage map.
...
A) More abstraction
B) Easier human reading

Prefer easier human reading.

ICF is optimized for maintenance and debugging.

A new developer should be able to open an Intent and immediately understand:

- What action is occurring
- Which stages are involved
- Which files contain the logic
...
Only two stage categories exist:

- core
- domain

Do not create sibling folders beside core and domain unless explicitly requested.

Program-specific logic belongs under domain.
