...
function bindGuestReview() {
  guestReviewForm.addEventListener('submit', async event => {
    ...
    const editToken = createReviewEditToken();
    ...
    const trustResult = evaluateReviewTrust(review, currentUserData, getRecentReviewContext());
    ...
    try {
      ...
      recordLocalLoyaltyEvent('review', {
        cafeId: currentCafeId,
        dedupeKey: `review:${reviewRef.id}`,
        xp: 15
      });
      if (review.imageUrl) {
        recordLocalLoyaltyEvent('photo-review', {
          cafeId: currentCafeId,
          dedupeKey: `photo-review:${reviewRef.id}`,
          xp: 20
        });
      }
      localStorage.setItem('mycafe:lastReviewAt', String(Date.now()));
      guestReviewForm.reset();
      const editUrl = getReviewEditUrl(reviewRef.id, editToken);
      reviewList.innerHTML = `
        <p class="text-green-700">${review.approved ? 'Review published automatically.' : 'Review sent for approval.'}</p>
