...
function bindEvents() {
  ...
  ownerProfileForm.addEventListener('submit', async event => {
    ...
    try {
      await updateDoc(doc(db, 'cafes', selectedCafe.id), updated);
  ...
  ownerSaveMenuBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'cafes', selectedCafe.id), { menu, updatedAt: new Date() });
      await refreshSelectedCafe();
      renderAll();
  ...
  ownerPhotoForm.addEventListener('submit', async event => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'cafes', selectedCafe.id), update);
  ...
  ownerPhotoGrid.addEventListener('click', async event => {
    ...
    try {
      if (removeButton) {
        ...
        await updateDoc(doc(db, 'cafes', selectedCafe.id), { photos, updatedAt: new Date() });
      }
      if (coverButton) {
  ...
  ownerDraftTranslationsBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'cafes', selectedCafe.id), { menu, translations: cafeTranslations, updatedAt: new Date() });
      await refreshSelectedCafe();
      renderAll();
  ...
  ownerReviewList.addEventListener('click', async event => {
    ...
    try {
      ...
      if (flagButton) {
        await updateDoc(doc(db, 'reviews', reviewId), {
          flagged: true,
          flagReason: 'Flagged by cafe owner',
          ownerFlaggedForAdmin: true
        });
      }
      currentReviews = await loadCafeReviews(selectedCafe.id);
      renderReviews();
