...
async function handleRsvpSubmit(event) {
  ...
  try {
    await updateDoc(doc(db, 'cafeCircles', currentCircle.id), {
      rsvp: arrayUnion(rsvp),
      rsvpCount: (currentCircle.rsvpCount || currentCircle.rsvp?.length || 0) + Number(rsvp.seats || 1)
    });
    status.textContent = 'RSVP saved and synced.';
  } catch (error) {
